...
明确了需求，下面开始理一理思路，这个组件可以用一个Popup组件来做。我们首先通过反射获取该脚本的所有方法信息（`MethodInfo`），然后进行筛选，只取出需要的，用它们的方法名创建一个`string`类型的数组作为`Popup`的显示选项，同时这个数组还要加一个空方法的元素（比如“`null`”）作为不选择方法的选项。
